...
public class BernieChen_ISP_improved extends PApplet {
...
public void keyReleased() {//Takes keyboard arrow input for the maze thru booleans mUp, mDown, mLeft, and mRight.
  if (key == CODED) {
    if (keyCode == LEFT) {
      mLeft = false;
    } else if (keyCode == RIGHT) {
...
public void gameMenu(int darkness) {
...
public String showScript(String script, int progress) { //Shows the "rolling" text.
  if (progress < script.length()) return script.substring(0, progress);
  else return script;
}
public void goodbye(int darkness) { //Shows the final goodbye message.
  textAlign(LEFT, TOP);
  rectMode(CORNERS);
  image(scroll, 0, 0);
  fill(0);
...
public void congratsMaze() { //Shows the congratulatory message on completion of the maze.
  endMenu = false;
  textAlign(LEFT, TOP);
  rectMode(CORNERS);
  image(scroll, 0, 0);
...
public void congrats() { //Gives congratulations to game completion.
  endMenu = false;
  textAlign(LEFT, TOP);
  rectMode(CORNERS);
  image(scroll, 0, 0);
...
public void retry() { //Gives a “Try Again!” message in the game of testing.
  endMenu = false;
  textAlign(LEFT, TOP);
  rectMode(CORNERS);
  image(scroll, 0, 0);
...
public void instructMaze(int darkness) { //Shows the instructions for the maze.
  textAlign(LEFT, TOP);
  rectMode(CORNERS);
  image(scroll, 0, 0);
  fill(0);
...
public void instructVault(int darkness) { //Shows the instructions for the game of testing.
  textAlign(LEFT, TOP);
  rectMode(CORNERS);
  image(scroll, 0, 0);
  fill(0);
...
public void splash() { //Shows the animation
  if (animTime <= 135) {
    background(0xffe7cfb4); //Scene 1: child lifts sweater over counter.
    noStroke();
    ellipseMode(RADIUS);
    rectMode(CORNERS);
    fill(200);
    ...
    fill(0xfffeebee);
    quad(81.5f, 107.5f, 83.5f, 117.5f, 58.5f, 122.5f, 56.5f, 112.5f);
    quad(-81.5f, 107.5f, -83.5f, 117.5f, -58.5f, 122.5f, -56.5f, 112.5f);
    ...
    background(0xff844c0c); //Scene 2: child slides coins over counter, shopkeeper takes the coins.
    stroke(100, 100);
    ...
    background(lerpColor(0xff87ceeb, 0xfffd5e53, animTime/1750.0f));
    rectMode(CORNERS);
    ellipseMode(RADIUS);
    fill(0xfff9d71c);
    noStroke();
    ellipse(432, (animTime-510)/3.0f, 100, 100);
    ...
    arc(0, -16, 60, 50, PI, TWO_PI);
    popMatrix();
    fill(0, (animTime-3500)/3.0f);
    rect(0, 0, 864, 864);
    fill(0xff635133);
    stroke(100, 100);
    rect(0, 0, 32+(animTime-3500)/2.0f, 864);
    rect(864, 0, 832-(animTime-3500)/2.0f, 864);
  } else if (animTime < 4600) {
    fill(0xff635133); //Scene fades.
    stroke(100, 100);
    rect(0, 0, 432, 864);
    rect(864, 0, 432, 864);
...
public void createCharlie() { //Loads the walking animations of the protagonist.
  for (int i = 0; i < 4; i++) {
...
public void createMenu() { //Loads the images for the menu background animation.
  for (int i = 0; i < 5; i++) menuForms[i] = loadImage("menu"+i+".png");
}
public void createTest() { //Loads the images for the vault image and the font.
...
public void setVarTest() {
  ...
  showWrong = false;
  letterInput = '_';
  code = (int) random(0, 8.9f);
  cipher = (int) random(0, 2.9f);
  clue = "Clue: ";
  ...
  if (cipher == 0) {
    ...
    int codeKey = (int) random(0, 25.9f);
    for (int i = 0; i < 5; i++) {
...
public void loadTest() { //Loads the game of testing.
...
public void vaultInput() { //Takes the keyboard input for the game of testing.
  if (letter1) {
    typed[0] = PApplet.parseChar(letterInput);
  } else if (letter2) {
    typed[1] = PApplet.parseChar(letterInput);
  } else if (letter3) {
    typed[2] = PApplet.parseChar(letterInput);
  } else if (letter4) {
    typed[3] = PApplet.parseChar(letterInput);
  } else if (letter5) {
    typed[4] = PApplet.parseChar(letterInput);
  }
  if (enterButton) {
    enterButton = false;
...
public void showClue(int x, int y) { //Shows the clues from the pedestals.
...
public void checkTorches() { //Updates the number of matches the protagonist has.
  matches = 0;
...
public void lightTorches(int x, int y) { //Updates torches' status.
...
public void setVarMaze() { //Initial variable assignment.
  torchesLit[0] = false;
  torchesLit[1] = false;
  torchesLit[2] = false;
  xpos = 1944;
...
public void updateCharlie() {
  charlieState = charlieForms[charlieDir*4+walkTime/25];
  image(charlieState, 402, 432-76);
}

public void mazeInput() { //Takes input for maze of learning.
  if (mUp) {
    ypos-=3;
    charlieDir = 3;
  } else if (mDown) {
...
public void loadMaze() { //Loads the nearby chunks from blueprint[] [] onto the screen.
...
public void readMaze(int x, int y) { //According to blueprint[] [], this program will show its corresponding image.
...
public void checkBoundary() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "BernieChen_ISP_improved" };
    if (passedArgs != null) {
